...
- [x] **P2 — Preserve submitted form values on validation errors.**
  ...
  That contract is what boosted the record create and edit form: success answers `204` with `HX-Location` for htmx and the unchanged `303 See Other` for a plain post, a refusal carries `CR-Form-Invalid` so the `htmx:beforeSwap` listener in `cr.js` swaps it, and `hx-disabled-elt` disables the submit button for the life of the request because HTML form posts are deliberately outside the `Idempotency-Key` contract. Three mutating forms deliberately stay native: delete (its confirmation is still an inline `onsubmit` handler that htmx's submit listener does not consult, so boosting it would delete a record after a declined confirmation; replacing it with `hx-confirm` is tracked with the other inline handlers in the Tailwind entry above), save-as-view (a refusal is still an error page), and the Kanban move form (its drag-and-drop equivalent submits with `form.submit()`, which fires no submit event, so both paths behave identically only while the rendered form is native too).
