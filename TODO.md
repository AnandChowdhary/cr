...
- [x] Opt-in record RBAC through a fixed-schema, audited `users` collection with database/collection/record inheritance, viewer/editor/access-manager/owner roles, current-principal CLI enforcement, filtered discovery, non-impersonable `--actor`, last-owner protection, and per-mutation access-decision evidence.
