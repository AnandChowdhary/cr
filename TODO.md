...
- [x] **P1 — Support private and shared records in one RBAC collection.**
  The `x-cr-access` JSON Schema extension opts an empty, history-free collection into creator-owned records. Every creation path atomically stores the active principal as owner with private visibility; collection editors retain discovery and create rights without inheriting content access. Record owners can publish read-only access to the active-principal audience, make the record private again, transfer ownership, or use ordinary direct record grants for narrower collaboration. Reserved `$cr_access` metadata is plaintext, schema-excluded, generic-mutation protected, and included in ordinary record audit diffs. Access decisions bind its hash independently from the user's policy hash, and list, search, raw reads, relations, views, HTTP, and audit filtering use the same fail-closed evaluator. Existing collections keep legacy inheritance unchanged, and activation refuses existing records or history so no migration can publish a partially owned collection.
...
- [ ] **P1 — Replace Tailwind Play CDN with compiled, pinned CSS, and declare a content security policy.**
  ...
  This is now the whole of what stands between these pages and a `default-src 'self'` policy, so the list is worth stating exactly. **One** the Tailwind Play CDN `<script src>` in `<head>`, which needs `script-src https://cdn.jsdelivr.net` and, because Play compiles CSS in the browser and injects it, `style-src 'unsafe-inline'` as well — so this single tag defeats both directives at once. **Two** the `GLOBAL_STYLES` inline `<style>` block, which needs `style-src 'unsafe-inline'` or a per-response nonce or hash; it is the server's own design system and is a much smaller problem than the CDN, but a nonce means the stylesheet stops being a constant and starts varying per response. **Three** one inline event handler: `onchange="this.form.submit()"` on the perspective selector, which needs `script-src 'unsafe-inline'`. It renders only under access control, has a `<noscript>` submit button beside it, and is also a WCAG 3.2.2 (On Input) failure in its own right — arrowing through the options with a keyboard navigates on every option — so moving it into `cr.js` and rendering the button unconditionally would fix an accessibility defect and remove the last handler in one change. **Four** the `data:image/svg+xml` favicon, which needs `img-src data:`.
  Nothing else is in the way. Both scripts ship from `/static/` with content-addressed names, the delete confirmation is a page rather than an `onsubmit` handler, no `hx-on:` attribute exists anywhere, and `tests/boosted_navigation_http.rs` asserts the absence of `onclick=`, `onsubmit=` and `hx-on` on every rendered page, so a fourth obstacle cannot be added without a failing test.
...
- [x] **P2 — Preserve submitted form values on validation errors.**
  A refused create or update is answered with the form instead of the generic error page: every value comes back in the control it was typed into as the exact submitted text, escaped — including the additional-attributes YAML, the raw front matter, and the Markdown body — rather than a reparsed round trip that would normalize `12500.50` to `12500.5` and reorder a mapping's keys. `Database::schema_violations` reports each schema failure with the dotted front matter path the schema locates it in, the same derivation `cr check` records in a finding's `field`, and the form renders each beside that control and marks it `aria-invalid`; a violation the schema locates in the record as a whole stays a form-level message rather than being guessed onto a field. The status is the status of the refusal — `422`, `412`, `409`, `400` — so a browser and the JSON API still agree about what happened, and a refused save keeps the `_expected_record_hash` it lost with, so losing a race cannot become a silent overwrite on the next click. Atomicity and audit neutrality are unchanged and asserted in every failing case in `tests/record_form_http.rs`.
  That contract is what boosted the record create and edit form: success answers `204` with `HX-Location` for htmx and the unchanged `303 See Other` for a plain post, a refusal carries `CR-Form-Invalid` so the `htmx:beforeSwap` listener in `cr.js` swaps it, and `hx-disabled-elt` disables the submit button for the life of the request because HTML form posts are deliberately outside the `Idempotency-Key` contract. Three mutating forms deliberately stay native: the delete form (now on the confirmation page; a refusal is a rendered error document, which htmx will not swap into a failed `POST`), save-as-view (the same reason), and the Kanban move form (its drag-and-drop equivalent submits with `form.submit()`, which fires no submit event, so both paths behave identically only while the rendered form is native too).

- [ ] **P3 — Give the save-as-view form the refused-form treatment.**
  Creating a view with a name already taken, or a Kanban layout with no grouping field, still answers with the generic error page and loses the column selection, filters and sort the reader had assembled. The record form's shape applies directly — re-render with the submitted values, keep the status of the refusal, mark the response `CR-Form-Invalid` — and that is what would let the form drop `hx-boost="false"`.

- [x] **P2 — Swap a view's results in place instead of its whole body.**
  Pagination, column sort, filter apply and search target `cr-view-table` and swap it `outerHTML`, so they change the rows and leave the sidebar, the heading and the filter panel alone; the panel staying open across an apply, with what was typed into it still in it, is the visible win and falls out of not re-rendering it. All four are still ordinary links and an ordinary `method="get"` form, and `hx-push-url="true"` pushes the request's own URL, so the enhanced path ends on exactly the URL the unenhanced one navigates to and every state stays shareable — `tests/targeted_swap_http.rs` asserts that by requiring the swapped region to appear verbatim in the same URL's document. `hx-swap` names the scroll behaviour rather than inheriting it: `show:top` for the cursor and sort links, which live inside the region and change which rows are on screen, and `show:none` for the filter and search form, because `htmx.config.scrollIntoViewOnBoost` would otherwise scroll the open panel off the top of the screen. The sort and cursor links gained ids so htmx's focus restoration returns the reader to the control they activated instead of to `<body>`, which is what a boosted body swap did.
  The two questions phase 2 left open are answered by an out-of-band swap and by the fragment envelope. The results fragment carries the heading's record-count pill and the filter disclosure's `<summary>` marked `hx-swap-oob="true"`, rendered by the same functions the heading calls so the patch cannot drift from the page it patches, and never marked that way in a whole document — where a boosted navigation would apply the patch and then deliver the page with the element missing. Widening the region was the alternative and it defeats the purpose, since the filter panel is in the heading; `data-active-filters` moved from the `<details>` to the `<summary>` for the same reason. Every fragment now leads with a `<title>`, which htmx lifts out, applies to the tab and removes before swapping — the only mechanism it offers, as there is no title response header. Nothing phase 4 swaps changes the title, which is exactly why it is sent structurally rather than remembered: `main-content` is a region of every page, and the first control that targets it across two pages would otherwise ship a stale tab.
  One behaviour had to be narrowed to make this safe. The `htmx:beforeSwap` listener in `cr.js` allowed a failed boosted `GET` to swap so a click on a stale link could land on the rendered 404 page; these controls are boosted elements that override `hx-target`, and htmx still reports them as boosted, so the clause now also requires the target to be `<body>` — otherwise a re-sort of a deleted view would paste a whole error document inside the table.
  Kanban moves are deliberately unchanged. The board *is* the region and a page turn already swaps it (`cr.js` rebinds the drag handlers on `htmx:load`, verified over CDP in `.context/phase-4/verification.txt`), but a move is a `POST` whose success is a redirect, and giving it a second success shape that returns markup would only serve the rendered form — the drag-and-drop twin submits with `form.submit()`, which fires no submit event — so the two ways of moving a card would stop behaving alike. That waits for the drag to go through htmx too.

- [x] **P3 — Announce what a swap changed, and ask before deleting on the server.**
  Every page renders one live region — `role="status"`, polite, atomic, visually hidden by the server's own stylesheet rather than by Tailwind's `sr-only`, and empty. It is rendered by the shell, which is the load-bearing part: it sits outside `main-content`, `cr-view-table` and `cr-record-form`, so it is an element an assistive technology was already watching when a swap arrives, where a region delivered inside a fragment would be created and filled in one step and announce nothing. The results fragment patches it with `hx-swap-oob="innerHTML"` rather than the `"true"` its two heading passengers use, for the same reason — `"true"` replaces the element — and a whole document never carries the patch. One sentence, the same for all four controls, stating the range the pager prints from the same `page_range`: "Showing records 26 to 33 of 33", or "No records match" when a filter empties the page. That is the answer to the note phase 4 left: after a filter apply focus correctly stays on "Apply view", so nothing reported that the rows behind it had changed, and after a turn to the last page `cr-page-next` no longer exists and focus falls to `<body>`, which ids could not fix. The `?notice=` banner is copied into the same region by `cr.js` one task later, because a message that arrives *with* a page is the one case a live region does not reliably announce; the banner and the impersonation strip both gave up their own `role="status"` so that a page has exactly one candidate for one announcement. Verified through CDP `Accessibility.getFullAXTree` in `.context/phase-5/verification.txt` — computed role, `live`, `atomic` and spoken text after every swap, plus the DOM node identity that proves the element survived it.
  Deleting a record is now a question the server asks. The record page links; the confirmation page carries the form, the CSRF token and the record's version, so no markup anywhere deletes on a single click, and `GET` and `POST` share one path with the `POST` contract unchanged. `hx-confirm`, which the plan proposed, was rejected: the previous `onsubmit="return window.confirm(...)"` is JavaScript, so a browser with scripting disabled already deleted the record on the first click with nothing asked — verified against `origin/main` in the same file — and `hx-confirm` is JavaScript too, so it would have preserved that hole exactly while appearing to fix it. A page is asked of everyone, can name the record and its collection and say what survives in the audit log, offers Cancel as a link rather than a button in a modal, and is the same two clicks the dialog was. The version travels from the confirmation page rather than the record page, which narrows the window the `412` exists for without closing it.
...
- [x] **P1 — Have `check` judge record access metadata the way writes do.**
  In a record-owned collection `check` now reads `$cr_access` with `RecordAccess::from_attributes`, the parser every write uses, and reports a missing or malformed value as a new `invalid_access_metadata` finding on the record with `$cr_access` as the field. The kind blocks `save`, so the divergence beside it escalates to an error the way it does for a schema violation. The schema still never sees the field, so a closed schema stays quiet and the finding is reported once. `check` also stops promising that `save` records the three divergences it refuses in a record-owned collection whatever the file holds: a hand-created file (ownership is recorded only by `cr create`), a hand-deleted one (a delete is authorized against the current owner), and a well-formed but changed `$cr_access` (managed through `cr access`). Each is an error that names the command to use instead, and the access change carries `$cr_access` as its field. `tests/check_cli.rs` covers each case and asserts that `cr save` really refuses it.

- [x] **P1 — Stop `check` from promising that `cr save` will record a direct edit to `users`.**
  Every divergence in `users` — modified, deleted, hand-added, or re-added after a deletion — is now an error saying that `save` refuses it because the collection is managed through `cr user` and `cr access`. It names `cr user restore <id>` for a modified or deleted user and `cr user add` for a hand-written registration, and no longer points at `status`. `users` records are validated against `users_schema()` rather than a schema file that cannot exist, and then with `User::from_attributes` once the schema passes, exactly the order `Database::validate` uses, so an unknown field is reported once and a duplicate grant the schema cannot express is still caught. An unusable schema now also escalates the finding for a deleted record, whose collection may never have been scanned.
...
- [x] **P1 — Idempotency keys for retried mutations.**
  CLI create/update/link/delete and REST record POST/PATCH/PUT/DELETE/link POST accept bounded, high-entropy retry keys. The raw key is never persisted: a principal/operation/record-scoped key hash, HMAC-SHA-256 of the lossless typed-YAML plaintext request keyed by the raw retry key, and original result are committed inside the same audit event and pending-mutation protocol as the write. Exact retries re-run current authorization and return that result without another event; protected results retain exact ciphertext Markdown/version durably and decrypt only for the response, while the stored safe relative path keeps replay stable across `data_dir` changes. Mismatched reuse is typed `idempotency_conflict`; concurrent callers serialize under the audit lock; pending recovery closes both response-loss crash windows; previews and failures consume nothing. The scoped identity occurs at most once regardless of request hash, enforced by replay, lookup, preparation, append, and recovery as typed `audit_integrity_failed`, while a pending copy of its already-committed event remains an honest cleanup case. Verification binds the stored result back to the replayed ciphertext event state, exact bytes, before/after version, collection, and ID, including delete tombstones. V1 is honestly single-record: multi-record save and sync, managed user/access lifecycle commands, and HTML form posts remain outside this contract until they have command/group result semantics; sync explicitly clears a caller retry key and `user ensure` remains independently declarative.
...
- [ ] **P1 — Audit retention, redaction, and encryption policy.**
  ...
  **Partially addressed and deliberately left open.** Collection schemas may now mark complete front-matter values and the Markdown body for transparent XChaCha20-Poly1305 storage. Keys remain external; envelopes carry key IDs for rotation; fresh nonces and portable database/coordinate-bound AAD prevent equality leakage and envelope swapping; records, audit snapshots, audit diffs, idempotency results, pending mutations, and durable protected sync streams contain ciphertext while CLI, REST, serve, validation, filtering, search, retry responses, and user-facing history operate on plaintext. Protected v3 run ledgers retain checkpoint presence and digests rather than raw JSON, recovery binds the decrypted stream checkpoint back to that digest, and lazy context creation refuses a validated interrupted protected run even before its first mutation. A stored per-sequence manifest makes marker removal or movement fail closed, pre-existing plaintext is refused rather than laundered through an audit migration that would keep the plaintext forever, and adapters neither inherit the keyring nor stage plaintext stdout on disk. Ordinary durable sync checkpoints and adapter stderr remain plaintext operational metadata and must not contain secrets. `audit verify` remains keyless because it verifies the stored ciphertext representation. This solves at-rest confidentiality only. Retention windows, redaction semantics, per-record/data-encryption keys and selective key destruction, an auditable key-provider interface, protected backup policy, and a stateful approval-plan protocol for fresh encrypted changes remain open.
...
- [x] Owner-only file-browser pins: a sidebar Browse section with All files and pinned locations, Pin/Unpin buttons, `cr pin add|remove|list`, and a lock-serialized `.cr/pins.yaml` that stores in-database paths relative to the root.
...
- [x] In-place table interaction: a view's pagination, column sort, search and filter apply replace only its results region, leaving the sidebar, the heading and an open filter panel untouched, while pushing the URL each one requested so every state stays shareable and works identically with JavaScript switched off; the heading's record count and applied-condition badge follow the swap as out-of-band elements, focus stays on the control that was activated, and a Kanban board is the same region as a table.
- [x] One polite live region per page, outside every region a swap can replace and patched rather than replaced, announcing what pagination, sorting, searching and filtering changed and carrying the success notice after a mutation; and a record deletion confirmed by a page the server renders, so the question is asked of a browser with JavaScript switched off too, which the inline handler it replaces never was.
