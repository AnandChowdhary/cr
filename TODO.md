...
- [ ] **P1 — Report a Markdown filename that cannot be a record ID instead of failing every command.**
  A single file whose stem is not a usable ID — `records/deals/..md`, say — makes `Database::record_files` return `id must be a non-empty path component`, which fails `status`, `save`, `audit verify`, `audit baseline`, and `sync run` with a message that names neither the file nor the collection. `cr list` does not validate the stem at all and happily returns the same file, so the two paths disagree about what a record is. `cr check` now reports it as an `invalid_record_name` finding and keeps going, which makes the database diagnosable, but does not fix the underlying inconsistency. Decide whether such a file is skipped with a warning or is a hard error everywhere, and make every caller agree.
