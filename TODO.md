...
- [ ] **P1 — Detect a forged newest event without an external checkpoint.**
  Every event but the last is pinned by the `previous_hash` of the event after it. The last one is pinned by nothing, and `audit verify` does not close the gap: it checks each change's `before` value against the state it replayed, and it compares `after_hash` against the file on disk, but it never checks the replayed *document* against `after_hash`. So the newest event's `actor`, `timestamp`, `message`, attribution, and the `after` value of every change can be rewritten and re-hashed, and `cr audit verify` reports the database clean while `cr audit log` shows the forged content. `tests/audit_corruption.rs::a_forged_head_event_is_accepted_by_verification_and_caught_only_by_a_checkpoint` asserts that behaviour rather than the behaviour we want; the `#[ignore]`d test beside it asserts the property that is missing.
  This is the same exposure as the removal of final events, which the threat boundary already names, and the same mitigation covers both: `audit head` plus `audit verify --expected-head`. It is listed separately because the fix is not the same. Cross-checking the replay against `after_hash` means re-rendering a replayed document to Markdown, which is not byte-identical for records introduced by `audit baseline` from files a human wrote, so it needs a decision about what a baselined record's replay is allowed to claim. Doing it during a *mutation* rather than only during `audit verify` is a second decision: a write currently replays the hash chain but not the per-record change sets, which is why a database whose replay is already inconsistent keeps accepting writes and only fails the next `audit verify`, reported against the innocent newer event.
  **Partially mitigated by the audit anchor below, and deliberately left open.** What the anchor changed is the *ergonomics* of the existing mitigation, not the missing property. `cr audit verify` now checks the head against `.cr-audit-head.json` by default, so the forgery is caught with no argument and no remembered checkpoint — but only because the forger left that file alone. The file sits at the database root and anybody who can rewrite `.cr/audit/` can rewrite it in the same pass; `tests/audit_corruption.rs::a_forged_head_event_is_accepted_by_verification_and_caught_only_by_a_checkpoint` now performs exactly that second write, and every one of its original assertions still holds. An anchor is an external checkpoint made automatic, so the `#[ignore]`d `a_forged_head_event_should_be_detected_without_an_external_checkpoint` stays ignored and its gap stays open: `cr` still cannot detect a forged head from the journal alone. Closing it needs the replay/`after_hash` cross-check described above.
...
- [x] Opt-in record RBAC through a fixed-schema, audited `users` collection with an application-owned profile namespace, human/service principals, atomic ensure/update/restore workflows, database/collection/record inheritance, viewer/editor/access-manager/owner roles, current-principal CLI enforcement, filtered discovery and audit history, verified owner delegation, non-impersonable `--actor`, last-owner protection, and per-mutation access-decision evidence.
