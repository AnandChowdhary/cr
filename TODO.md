...
- [ ] **P1 — Have `check` judge record access metadata the way writes do.**
  In a record-owned collection every write reads `$cr_access` with `RecordAccess::from_attributes` before the schema runs, so a missing or malformed value is refused, and `cr save` refuses a direct edit that introduces one (`record has invalid '$cr_access' metadata`). `check` never reads the field: `database::schema_attributes` removes it before schema validation, which is correct, and nothing else looks at it. A hand-edited `visibility: bogus` therefore reaches `check` only as a `record_content_mismatch` warning saying `cr save` records the change, and the run exits 0 while `save` refuses. Report it as a save-blocking error on the record with `$cr_access` as the field, so the divergence beside it escalates the way it does for a schema violation. A closed schema (`additionalProperties: false`) used to catch this by accident, by rejecting every record in the collection; with that gone, this check is the only thing that would.

- [ ] **P1 — Stop `check` from promising that `cr save` will record a direct edit to `users`.**
  `cr save` refuses every direct edit to the reserved `users` collection (`the users collection is managed through 'cr user' and 'cr access'`), whatever the edit contains. `check` reports one as the ordinary `record_content_mismatch` warning saying `cr save` records the change, and exits 0. It also never validates `users` records against the built-in schema writes use: `.cr/schemas/users.json` cannot exist, so its schema cache finds no schema and a hand-added top-level `nickname:` passes. Report a divergence in `users` as an error that points at `cr user` and `cr access` rather than `status`, and validate `users` records against `users_schema()` as `Database::schema_check` already does. A user record `User::from_attributes` cannot read for the acting principal already stops every command, `check` included, before any of this runs.
