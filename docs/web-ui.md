...
A board is not paged like a table. Each lane shows up to the view's page size of its own records, in the board's order, and its heading counts every record it holds in the view, beside a dot in its state's colour (the same words colour it as colour a badge). A lane holding more than it shows ends in **Show N more**, which shows more of every lane — twice as many, up to the server's `--max-page-size` — by swapping the board alone; past that most, the lane says how many are left for a filter to reach. Under the board, one line says how many records of the whole it shows.
