...
A saved view's bar has **Edit view** beside **Save as view**. It opens a page at `/<view>/edit` with every setting of the definition: its title, its conditions in the same filter builder the **Filter** panel uses, the default sort, the columns or card details, the layout and Kanban grouping field, and the rows per page. **Save changes** overwrites `.cr/views/<name>.yaml` in place, so the view keeps its name, route, and collection; a view of another collection is another view, made with **Save as view**.

The editor opens on the view as you were looking at it. Filters applied on the page join the view's own conditions, and the page's sort, column choice, and rows per page are preselected, so narrowing a view and then choosing **Edit view** › **Save changes** makes the narrowed view the saved one. Opened from a plain link, the editor shows the definition alone.

A definition's `filters`, `where_expr`, and every group that matches **all** of its conditions become one list in the builder, because all of them must hold. A group matching **any** of several conditions cannot join that list without changing what the view matches, so it is listed under **Also required** as one sentence with a checkbox, and unchecking it removes the whole group. Anything left as it was is written as it was: changing only the title of a hand-written definition keeps its `filters`, `where_expr`, `sort_by`, and `columns` exactly as spelled, and a view that picks its own columns goes on picking them unless you change the selection. A setting the server refuses, such as a Kanban layout without a grouping field, returns the editor with the reason above it and everything you typed still in it; nothing is written.

**Delete view…** at the bottom of the editor asks on a confirmation page first, like deleting a record. Deleting removes only the definition file: every record stays, and a saved view that shared its collection's name gives the route back to the collection's automatic view. Afterwards the browser lands on that collection. Editing and deleting need the same ownership as saving, and the controls are not rendered for anyone else.

From the command line, `cr view delete NAME` does the same:

```sh
cr view delete interviews
```
...
A view's own controls use that seam rather than replacing the page. Next and Previous, the column sort links, the search box and the filter panel's “Apply” all target `cr-view-table`, so turning a page, re-sorting, searching or applying a filter changes the rows and leaves the sidebar, the heading and the panel alone—the filter panel now stays open across an apply, with what was typed into it still in it, instead of closing on a reload. Each of them is still an ordinary link or an ordinary `method="get"` form, and each pushes the URL it requested into the address bar, so every filtered, sorted and searched state remains shareable and a stranger with JavaScript switched off gets the same rows from the same link. The two facts in the heading that a change of result set changes—the record count, and the badge counting applied conditions—travel with the results as out-of-band elements, rendered by the same code the heading uses, so they cannot drift from it. So do the view controls that act on the page's state: the hidden fields **Save as view** submits and the **Edit view** link, so saving or editing after an apply, a re-sort, or a column change uses what is on screen rather than what the page was loaded with. Focus stays on the link that was activated rather than falling back to the top of the document, and that link's label is re-rendered by the same swap, so “sort by name ascending” becomes “sort by name descending” where a screen reader is already reading. Moving a Kanban card is deliberately still a plain form post: the board is the same region and a page turn does swap it, but a move is a `POST` answered with a redirect, and dragging a card submits a form built in JavaScript that htmx never sees—giving only the rendered form a swap would make the two ways of moving a card behave differently.
