...
View filter forms submit a `filter_match=all|any` mode and repeated `filter_field` and `filter_value` query pairs. The server requires a one-to-one pairing, caps each request at 20 conditions, parses every value with the same typed equality semantics as CLI `list`, and combines the ad hoc conditions with AND or OR before paginating. Saved-view filters are a separate required AND scope, so OR matching cannot broaden a record set beyond the saved view. Generated controls use schema enum, boolean, number, and string-format metadata; a small progressive script adds/removes rows and swaps the value control when a field changes, but URLs and server behavior do not depend on that script. Pagination preserves the match mode and all pairs in order.
