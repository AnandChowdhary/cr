...
### The server's verified journal

Every read that needs audited state verifies the chain: listing a plaintext collection replays it to check that no record once owned encrypted storage, a view's created and updated columns walk it, and under RBAC every request re-derives the principal's audited policy from it. A CLI command does that once. A server did it several times per request, and each time over the database's whole history, so pages grew slower with every event ever recorded.

`cr serve` therefore keeps the result of one verified walk for as long as it runs (`JournalCache` in `src/audit.rs`), and the next reader verifies only what was appended since. The walk's checks — sequence continuity, the hash chain, version progress, idempotency identities — are one resumable `ChainWalk`, so an event verified late is held to exactly the rules a fresh walk applies. The server starts the first walk in the background as soon as it is listening, and a request that arrives sooner waits on that walk instead of starting another.

What the cache trusts in place of re-hashing is explicit. The newest segment, which every append rewrites, is compared byte for byte with the prefix already verified. An older segment is not read again while its path, device and inode, length, modification time, and change time are unchanged; any write, truncation, or replacement changes one of them, and the change time cannot be set back by `touch` or any other ordinary interface. A change anywhere sends the next reader back to the first segment, and a walk that fails caches nothing. The trade is that a same-length rewrite of a sealed segment that also restores its change time — by resetting the clock, writing the block device, or landing in the same tick of a coarse filesystem clock as the segment's last legitimate write — goes unnoticed by that server until it restarts. Verification never takes the shortcut: `cr audit verify`, `cr check`, and their API routes walk the whole chain every time, and every write verifies the whole chain before it extends it, so a mutation never builds on a journal only the cache believed.
...
- A running server verifies the journal once and afterwards only what is appended, trusting a sealed segment's file identity and times in place of re-hashing it. A same-length rewrite that also restores the segment's change time is noticed by that server only after a restart; `audit verify`, `check`, and every write still verify the whole chain. See [the server's verified journal](#the-servers-verified-journal).
