...
The HTML `Save as view` POST copies the source view's immutable scopes and presentation settings, appends the currently applied browser group without flattening its Boolean mode, and persists the effective sort. It uses the same CSRF token and atomic no-clobber file creation as other local forms. Search text is not persisted in this version and stays in the shareable query URL. Creating a view changes configuration rather than record data, so it does not append a record audit event; configuration history remains an explicit backlog item.
