...
The saved walk is derived state and as writable as the journal: whoever can rewrite `.cr/` can write one that claims a different replayed state for segments that have not changed, and a read that resumes from it believes that claim. Three things bound that. A walk is only saved if the saving process verified it from the first event — which the admission walk every write makes already does — so one saved walk is never built on another, and the next write replaces a forged one with a verified one. Nothing that decides whether the journal is intact uses it: `cr audit verify`, `cr check`, and their API routes walk the whole chain every time, and the one walk every write makes before it extends the chain starts from the first event, is never taken from a cache, and is the walk the write appends to and saves, so a mutation never builds on a chain only a cache believed. And `--verify-audit` makes any other command start from the first event too. The checks that do resume from it — whether a record once owned encrypted storage, whether a working user file or link target is its audited version — exist to catch a hand edit that did not go through `cr`, and a saved walk that `cr` wrote still catches it. Getting past them takes a deliberate forgery of `.cr/`, as it always took a deliberate forgery of the journal, and the hand-edited file is still reported by `cr status` and `cr check`.
...
- A read verifies only what was appended since the walk the last write saved, trusting a sealed segment's file identity and times in place of re-hashing it, and a running server does the same for its own walk. A same-length rewrite that also restores the segment's change time goes unnoticed until the next walk from the first event, and a forged saved walk is believed until the next write replaces it; `audit verify`, `check`, `--verify-audit`, and the admission walk of every write still verify the whole chain. See [the verified journal](#the-verified-journal).
- A single-record write walks the chain once, under the audit lock, and its append checks the journal against that walk as a cached walk is checked rather than walking it again. The lock keeps cooperating writers out of the milliseconds in between; anything else that changes a verified segment sends the append back to the first event, except the same-length rewrite that restores a sealed segment's change time, which the write appends to and the next walk from the first event reports. See [the audit protocol](#audit-protocol).
