...
The HTML layer follows the web platform directly. Native forms submit mutations with `POST`, successful writes use `303 See Other` to return to an idempotent `GET`, a refused write is answered with the form and the status of the refusal, and Maud escapes dynamic text while producing server-rendered markup. Enhancement is layered on top of that rather than substituted for it: a vendored copy of htmx boosts navigation so a link swaps the body instead of rebuilding the document, a view's own pagination, sort, search and filter controls narrow that to the results region alone, one live region reports what a swap changed to a reader who cannot see it, and every route still answers a browser with JavaScript switched off, because htmx intercepts markup that already works and because a destructive action is confirmed by a page the server renders rather than by a dialog a script opens. Tailwind's Play CDN supplies the requested no-build styling, with its documented development-only limitation recorded as technical debt.
