...
The HTML layer follows the web platform directly. Native forms submit mutations with `POST`, successful writes use `303 See Other` to return to an idempotent `GET`, a refused write is answered with the form and the status of the refusal, and Maud escapes dynamic text while producing server-rendered markup. Enhancement is layered on top of that rather than substituted for it: a vendored copy of htmx boosts navigation so a link swaps the body instead of rebuilding the document, a view's own pagination, sort, search and filter controls narrow that to the results region alone, one live region reports what a swap changed to a reader who cannot see it, and every route still answers a browser with JavaScript switched off, because htmx intercepts markup that already works and because a destructive action is confirmed by a page the server renders rather than by a dialog a script opens. Styling is Tailwind CSS compiled ahead of time by a pinned release and served from the binary, so no page depends on another origin.
...
- Relations live under `relations.<name>` as lists of `{ collection, id }` references. `cr link` verifies the target exists and is idempotent. `cr unlink` removes every reference matching a `collection` and `id`, prunes a relation and then `relations` once empty, and does not read the target, so it can remove a dangling reference and needs no read access to the target. Both are recorded as the `link` action, with the change set saying which way the reference moved: a new `unlink` action would have made every earlier `cr` refuse a journal containing one, since `AuditAction` is deliberately closed. For the same reason an unlink's retry key is scoped to the `link` operation, with the request itself marked as an unlink.
- A collection schema is optional and validates only its front matter. Its `x-cr-ui` extension carries presentation hints—form field order, a navigation label, an icon, the field that names its records—that never change what validates, and a hint that does not fit is ignored rather than refused.
...
- a versioned exact-Markdown post-state witness whenever a version 3 event leaves the record present;
- the previous event hash, chaining events across segment files.

Create and baseline events store the complete after-state. Delete events store the complete before-state. Updates and links recursively diff objects while treating arrays and scalar values as replaceable units. Version 2 changes use explicit `add`, `remove`, and `replace` operations, so an absent value remains distinguishable from a present `null`. Version 3 adds `after_snapshot: {version, markdown}` to every event whose post-state is present. The reader accepts and safely converts version 1 change objects while retaining their original hashed bytes.

Verification replays these operations independently for each record. Every event's `before_hash` must equal that record's prior audited hash, every change's before-value must equal the replayed semantic state, and the action must match the resulting presence transition: create and baseline are absent-to-present, update and link are present-to-present, and delete is present-to-absent. Every v3 present state requires an exact snapshot whose Markdown parses to that same semantic state and whose exact bytes hash to `after_hash`. Baseline is valid only as a record's first event.

Version 1 and 2 could hash bytes they did not retain: baselines, direct filesystem saves, and even an ordered managed-user mapping can contain representation details audit JSON loses. Rejecting those journals would be backwards-incompatible, while re-rendering them and claiming exact equality would be false. Replay therefore marks a v1/v2 event whose render does not match as a legacy representation gap. A later strongly verified event clears the gap. If it remains at the record head, the materialized record may close it only when its exact hash already equals `after_hash`; parsing those bytes must then yield the replayed semantic document. A missing or directly edited file remains an ordinary reconciliation failure. Payload versions may increase but never decrease, so a v3 journal cannot switch back to the legacy exception midway. No internal rule can prove that the first event, or every event, was not rewritten from v3 to v2 together with its hashes; detecting that all-history downgrade still requires an externally retained head checkpoint.

The same replay runs before every mutation and pending-mutation recovery. A journal that already contains an inconsistent change set therefore refuses the write before a pending file, record replacement, or innocent later event can be created. The typed failure is `DomainError::AuditIntegrity`, exposed as `409 audit_integrity_failed`, and names the guilty sequence without a path or field value.
...
**What this buys, stated plainly.** Replay now catches a head event whose rewritten changes do not reproduce its record hash, even when the attacker recomputes the event hash and local anchor. It cannot derive actor, timestamp, message, authorization, intent, or attribution from record state, and cannot detect an internally consistent rewrite that updates the record bytes and every claimed hash. A file at the database root is writable by anybody who can write `.cr/`, so the attacker can still change those facts and the anchor in one pass. `tests/audit_corruption.rs::a_forged_head_actor_still_needs_an_external_checkpoint` performs exactly that pair of writes and passes local verification. The remaining protection comes entirely from **committing the anchor to Git**: a pushed, distributed history is a second write boundary that a local filesystem write cannot reach, and a reviewer who sees the head hash change in a diff that contains no records — or sees it change to a value nobody's working copy produced — is looking at the tamper. The feature being shipped is ergonomics and a default-on check, not a new cryptographic guarantee.
...
Adding the attribution fields did not change audit version 2. Every one is `Option` with `skip_serializing_if`, so an event with no attribution serialized to exactly the bytes it did before, and verification hashes stored bytes rather than a reserialization, so existing chains kept identical head hashes. Version 3 was introduced later for a semantic reason an old verifier does not implement: exact post-state replay. The serialized snapshot field remains optional because deleted states have no post-state bytes and legacy events must still deserialize, while verification requires it for every v3 present state. The cost is intentional: an older binary refuses a v3 journal rather than reporting it verified without checking the new invariant. The payload still must never gain an unversioned non-optional field, a `HashMap`, or a `#[serde(flatten)]`.
...
A single-record write verifies the chain once, after it takes the lock — from the first event, or by resuming the walk the last write saved (see [the verified journal](#the-verified-journal)) — and reads what it needs to know about the records it touches from that walk: the record's last audited hash, whether it once owned encrypted storage, a link target's audited version, and whether a user's ID was deleted. It used to walk the chain a second time immediately before appending. The lock has been held since the walk, so no cooperating writer can have appended in between, and the append instead checks that the journal on disk is still what the walk verified: every sealed segment at its path with the file identity, length, and modification and change times it had, the newest segment beginning with exactly the bytes verified, and anything after them verified by the walk's own rules. Anything already verified that has changed is judged by a walk from the first event instead. The head on disk must then still be the head the event was prepared on, and the event is replayed against its record's verified state before a byte is written, as before. That is the test a cached walk must pass before it is trusted ([the verified journal](#the-verified-journal)), with the newest segment compared byte for byte rather than by digest, so it makes that section's one trade over a much smaller window: a same-length rewrite of a sealed segment that also restores its change time, landing in the milliseconds between a write's walk and its append, gets the event appended rather than refused, and the next walk from the first event reports the rewrite. Bulk `save` has always relied on the same lock, with one walk for every record it accepts. Pending recovery still walks twice, once to decide and once to append, because it runs once, after a crash, and costs nothing worth saving. Walking from the first event, a write costs one walk's worth of time in proportion to history rather than two: on 2,300 events a release build's `update` went from about 470 ms to 240 ms, beside about 230 ms for the walk alone. Resuming, it costs the events appended since the walk it resumes.

### The verified journal

Every read that needs audited state verifies the chain: reading or listing a plaintext collection replays it to check that no record once owned encrypted storage, a view's created and updated columns walk it, `--as` re-derives the operator's and the target's audited policy from it, and under RBAC every server request does the same for its principal. Done from the first event every time, each of those costs more with every write the database has ever had: a server did it several times per request, and a short-lived `cr get` spent its whole run on it.

A process therefore keeps the result of one verified walk for as long as it runs (`JournalCache` in `src/audit.rs`), and the next reader verifies only what was appended since. `cr serve` keeps one for its lifetime, and a CLI command keeps one for its own, so `--as` and the command it delegates share a walk. The walk's checks — sequence continuity, the hash chain, version progress, idempotency identities, and the replay of every change set — are one resumable `ChainWalk`, so an event verified late is held to exactly the rules a fresh walk applies. The server starts its first walk in the background as soon as it is listening, and a request that arrives sooner waits on that walk instead of starting another.

What the cache trusts in place of re-hashing is explicit. The newest segment, which every append rewrites, is compared with the prefix already verified. An older segment is not read again while its path, device and inode, length, modification time, and change time are unchanged; any write, truncation, or replacement changes one of them, and the change time cannot be set back by `touch` or any other ordinary interface. A change anywhere sends the next reader back to the first segment, and a walk that fails caches nothing. The trade is that a same-length rewrite of a sealed segment that also restores its change time — by resetting the clock, writing the block device, or landing in the same tick of a coarse filesystem clock as the segment's last legitimate write — goes unnoticed while the walk is trusted.

The walk also outlives the process. Every command that appends an event saves its walk in `.cr/cache/verified-journal.json` while it still holds the audit lock, and the next command, CLI or server, starts from that file rather than from the first event, so a read costs about the same on the ten-thousandth event as on the tenth. The file records the newest segment's verified prefix as a length and SHA-256 rather than the bytes, carries a digest of its own body, and names the `cr` release that wrote it; a file that is torn, damaged, from another release, or no longer describes the segments on disk is ignored, and so is one whose continuation fails, in which case the walk from the first event is the answer. A saved walk can therefore make a command skip work but never make it fail. Reads never write it, so `get`, `list`, previews, and `check` leave the database byte for byte as they found it. `.cr/cache/` holds nothing else, can be deleted at any time, and contains a `.gitignore` that keeps it out of a `git add .`, because it is rewritten on every append and its segment identities describe one machine's files.

Writes resume it too. A write that finds a saved walk, or a cached one in the same process, brings it up to date by verifying the events appended since, prepares against it, appends to it, and saves it, so a write costs what those events cost rather than what the whole history does. Every walk records how long the journal was when it, or the walk it descends from, last verified every event from the first, and once the journal has grown past that by `audit.full_walk_after_events` (64 by default) the next write walks from the first event instead. On a release build, 64 consecutive `update`s took a median of 23 ms each on both 1,000 and 2,300 events, against 122 ms and 244 ms when every write walked from the first event; the one write in 64 that does still costs a walk, about 250 ms on 2,300 events, which brings the mean to about 30 ms.

The saved walk is derived state and as writable as the journal: whoever can rewrite `.cr/` can write one that claims a different replayed state for segments that have not changed, and a command that resumes from it believes that claim. Three things bound that. Nothing that decides whether the journal is intact uses it: `cr audit verify`, `cr check`, and their API routes walk the whole chain every time. A forged walk steers writes only until the next write that walks from the first event, at most `full_walk_after_events` events later; that walk replays the real history, so an event appended on a state only the forgery claimed is refused there, by sequence, as it is by `cr audit verify`, and so is every write after it until the journal is repaired. And `--verify-audit` makes any command, write or read, start from the first event, while `audit.full_walk_after_events: 1` does so for every write, which is how a write behaved before it could resume. The checks that do resume from it — whether a record once owned encrypted storage, whether a working user file or link target is its audited version — exist to catch a hand edit that did not go through `cr`, and a saved walk that `cr` wrote still catches it. Getting past them takes a deliberate forgery of `.cr/`, as it always took a deliberate forgery of the journal, and the hand-edited file is still reported by `cr status` and `cr check`.
...
nonblocking with `O_NOFOLLOW` on Unix, checked again through the resulting file
descriptor, escaped into HTML, and bounded to 1 MiB (4 KiB for hex-rendered
...
outer authority. A directory's README and `SKILL.md` are previewed through
that same path — chosen only among regular entries, so a symbolic link named
...
The sixteen kinds are `dangling_link`, `malformed_relation`, `schema_violation`, `invalid_access_metadata`, `unusable_schema`, `invalid_record_name`, `unreadable_record`, `unaudited_record`, `missing_record`, `record_content_mismatch`, `audit_chain_broken`, `approval_mismatch`, `interrupted_sync_run`, `audit_anchor_mismatch`, `audit_anchor_behind`, and `audit_anchor_missing`. Relation references tolerate extra keys beside `collection` and `id`, because an annotated reference is still a reference and a check that fires on a working database is worse than no check.
...
`status` is the working-tree view. It answers *what would `cr save` record next?*, and every row it prints is an expected, resolvable direct edit. `check` reports the same three physical conditions — a record with no audit history, an audited record whose file is gone, and a file whose bytes do not match the audited state — because a whole-database integrity report that omitted them would be misleading. It reports them at `warning` and points back at `status`, **provided `cr save` could actually reconcile them**. When the same record also fails to parse, fails its schema, carries unreadable `$cr_access` metadata, or cannot be named, `save` refuses it, the divergence is permanent until a human intervenes, and the finding is escalated to `error`. `check` judges records the way writes do: a record-owned collection's `$cr_access` metadata is read with the same parser writes use, and `users` records are validated against the built-in users schema and then as principals. Some divergences `save` refuses wherever they occur rather than for what the file holds — any direct edit to `users`, and a creation, deletion, or `$cr_access` change in a record-owned collection. Those are errors too, and their message names the command that does resolve them (`cr user restore`, `cr create`, `cr delete`, or `cr access`) instead of pointing at `status`. That is the boundary: `status` enumerates the working set, and `check` says whether the working set is reconcilable and whether the journal underneath it is sound. Everything else `check` reports — dangling links, malformed relation values, schema drift, unusable schemas, invalid names, chain damage, approval mismatches — is invisible to `status` entirely.
...
The journal contains historical values and deletion tombstones. Version 3
baselines and non-derivable representations also retain the complete exact
post-state snapshot, so an event that changed one field can retain unchanged
fields when its YAML bytes were non-canonical. Encryption, redaction rules,
retention, and non-bypassable backing-store isolation remain deployment
concerns and must be designed before storing regulated or highly sensitive
data.
...
The REST API uses generic collection and record routes. List, search, status, and audit-log endpoints support bounded `limit`/`offset` windows. Server-rendered tables page differently: they carry a record-ID cursor (`after`/`before`) because they open newest-first, where an offset is the wrong address — one insert shifts every later row and repeats one the reader has already seen. The complete ordered result is assembled before the page is cut from it, so a cursor page still reports an exact position and total, and a cursor naming a record that no longer matches resolves to the first page. `offset` remains accepted on those routes so older links resolve. Record scans can report an exact total. Audit history intentionally reads only `offset + limit + 1` recent matching events, so its total is unknown while `has_more` remains exact; this preserves the segmented journal's bounded-read design.
...
`.cr/pins.yaml` stores a format version and an ordered list of `{path, label?}` pins for the file browser's sidebar. Unlike views it is one file, because the list's order is the sidebar's order, so read-modify-write is serialized by `.cr/pins.lock`; without it two concurrent pins would both read the old list and the second write would drop the first. Paths are normalized lexically and stored relative to the database root when they lie inside it. Like views, it is configuration rather than record data and has no audit history.

`.cr/views/<name>.yaml` stores a format version, title, target collection, typed equality `filters`, richer shared `where_expr` predicates, bounded structured `filter_groups`, visible dotted columns, layout, optional Kanban grouping field, optional default sort field and direction, and default page size (10 unless set). A definition with no `sort_by` inherits the newest-first default, `$created_at` descending. Equality and `where_expr` predicates are an immutable AND scope. Every structured group preserves its own all/any mode, and groups are ANDed with that base and with the browser's current ad hoc group, so a URL cannot broaden the configured view. These files contain no record data. A saved view overrides the automatic view with the same route name; otherwise each discovered collection is available at `/<collection>`. Saved and automatic views whose collections the effective principal cannot discover are omitted. View names reserve their single-segment root routes, while `/health`, `/audit`, `/browse`, `/openapi.json`, `/perspective`, `/static`, `/users`, and `/api` remain server-owned. Absent query sorting inherits the saved default; an explicitly empty `sort_field` clears it for that URL, while any chosen field overrides it. Legacy view files deserialize with no expression predicates or groups, no default sort, and ascending direction. A saved definition can be replaced in place — same name, same collection, validated like a new one, written with an atomic rename — or deleted, which removes only the file; an automatic view has neither operation. The browser editor folds a definition's `filters`, `where_expr`, `all` groups, and single-expression groups into the one list its filter builder edits, keeps multi-expression `any` groups as separately removable groups so no edit changes a view's meaning by accident, and writes back the stored spelling of any part it did not change.
...
The root page, tables, search/filter controls, pagination, record forms, embedded record history, and global `/audit` timeline are rendered with Maud on the server. Every HTML handler also loads the effective principal's discoverable views and passes them to one workspace shell. Desktop pages render saved views first, then collections, in a persistent sidebar beside utility links and the RBAC perspective selector: a saved view is a decision someone made about what matters, so it outranks the automatic page for every collection. Mobile pages render the same routes in the same order as a sticky, horizontally scrollable strip. CR's own `users` registry is not a view, so it is offered separately: a perspective that may read access policy gets an **Internal** sidebar section and an index section linking `/users`, a page that lists every principal, grant, and profile field and renders no mutation control at all. Database owners get a **Browse** section after the collections: **All files** links to `/browse`, followed by the locations pinned in `.cr/pins.yaml`, each resolved per render with one `canonicalize` and one `metadata` call (the domain caps the list at 50 for that reason) and linked by its canonical location, which is also how a browse page addresses itself, so the active entry is a string comparison. Access managers see none of it, because policy inspection and host-filesystem inspection are different authorities. A pins file that no longer parses is reported inside the section rather than failing the page, since a typo in a navigation preference must not lock an owner out of the UI. Changing a user stays with `cr access`, `cr user`, and the REST API, which enforce the reserved-field rules the browser forms cannot express. Active-route state is computed on the server, so navigation remains correct without client application state. Under RBAC, the same policy evaluator determines both sidebar visibility and collection and record visibility, plus whether new-record, save-view, edit-view, edit, delete, and per-card Kanban movement controls render. A readable but non-editable record is rendered through a disabled fieldset with an explicit read-only label. Dynamic schema metadata, title, front matter, ID, historical values, actor, message, and error text are HTML-escaped. Before/after previews are character-bounded in HTML while the authoritative complete events remain available through the CLI and JSON API. Styling comes from Tailwind utilities compiled ahead of time rather than a JavaScript framework: `scripts/tailwind.sh` runs a pinned, checksum-verified Tailwind CSS CLI over the markup, the output is committed as `src/static/tailwind.css` and embedded, and CI regenerates it to prove the committed file is current. Building `cr` therefore needs neither Node nor a network, and the UI works offline.

The pages' own JavaScript is an embedded asset rather than markup. Two files are compiled in with `include_str!` and linked from `<head>` with `defer`, so both still run against a parsed document: `src/static/cr.js`, which holds the filter builder, the save-as-view layout control, Kanban drag and drop, and htmx's configuration; and `src/static/htmx-2.0.10.min.js`, the published htmx release vendored byte for byte, with its SHA-256 recorded beside the constant and its Zero-Clause BSD text committed alongside it. There is no CDN fetch, no npm tree, and no build step: the release is a file in the repository, and re-pinning it is a commit that changes a name a test asserts literally. `GET /static/{file}` matches the requested name against the constants the binary was built with and performs no filesystem access, so a traversal has nothing to traverse and the hardened path walk in `src/paths.rs` has no static-file equivalent to maintain. Each served name embeds a truncated SHA-256 of the contents, which is what makes `Cache-Control: public, max-age=31536000, immutable` honest: an edit to a file changes its URL, and a version number nobody remembered to raise cannot strand a browser on a stale copy. The route sits outside the authorization layer alongside `/health`, because it carries no database data and because a `<script src>` cannot attach the bearer header `CR_API_TOKEN` requires, so an authenticated asset route would silently disable the enhancements in precisely the deployments that set a token. Keeping the scripts out of the documents is also the prerequisite for a `script-src 'self'` content security policy, which the inline blocks made unreachable.

Navigation is boosted rather than reimplemented. `hx-boost="true"` on `<body>` makes htmx answer a same-origin link or read-only form with a request whose response replaces the body's contents, so a sidebar click stops discarding the parsed stylesheet, the scroll position, and the enhancements already bound; `<title>`, the address bar, and the back and forward buttons follow the response, because the response is the same complete document a reload would have fetched. No handler knows about any of this, and a boosted navigation is answered with exactly the document a browser is, byte for byte, which is what keeps the HTTP suite — which never sends an htmx header — a real no-JavaScript regression suite. A body swap fires no `load` event, so `cr.js` re-runs its enhancements on every `htmx:load` and records which elements it has already bound in a `WeakSet`, not in a `data-` attribute that an HTML history snapshot could resurrect as a lie. htmx's history cache is switched off: it writes rendered pages into `sessionStorage`, which would contradict the `Cache-Control: no-store` every page carries, so back and forward re-request instead, which is still a swap rather than a reload. A one-line progress bar, styled in the server's own sheet from the `htmx-request` class htmx applies for the life of a request, keeps a slow journal walk from reading as a dead click, and collapses to a static strip under `prefers-reduced-motion`.

One URL, two envelopes. Each page renderer composes its content and then hands it to one helper that decides how much to send: the content inside the workspace shell, or the content alone. A request asks for the smaller answer by being an htmx request — `HX-Request: true` — that names, in `HX-Target`, the DOM id of a region the route renders: `main-content`, the `<main>` every page's content fills, or `cr-view-table`, a view's results region, which is the table and its pager in a table view and the grouping caption and board in a Kanban one, so the controls both layouts share can point at one id. The fragment is not a second rendering; it is the same markup the document puts in the same element, which is why "what may this principal see" is still decided in exactly one place. It leads with a `<title>`, which htmx lifts out of a response, applies to the tab, and removes before swapping — the only mechanism available, since htmx has no title response header, and worth sending even where the title cannot have changed, because the URL a swap pushes is bookmarkable and the two have to agree about what the state is called.

The seam only ever narrows the answer when the request said which context it lands in, because the two failure directions are not symmetric: a whole document swapped where a fragment was expected is a visibly broken page, while a fragment swapped into `<body>` is a page that has quietly lost its navigation and has nothing left to boost from. So a browser address bar, a `curl`, a request with a stale `HX-Target` and no `HX-Request`, a boosted navigation — whose target is `<body>`, which has no id, so htmx sends no `HX-Target` at all — and a target no route renders all get documents. A back or forward restore gets one for three independent reasons: `cr.js` switches off `historyRestoreAsHxRequest` so a restore is not labelled an htmx request, htmx sends no target on one, and the server refuses a fragment to anything carrying `HX-History-Restore-Request`. Rendered error pages stay whole documents too, since htmx declines to swap a non-2xx response except for the boosted navigation case above. Every HTML answer lists `Cookie`, `HX-Request`, `HX-Target`, and `HX-History-Restore-Request` in `Vary` — appended by whichever layer runs second, never replacing what the other wrote — so a shared cache cannot serve a fragment to a request that asked for a page, even though `Cache-Control: no-store` already forbids storing either.

A view's own controls use that seam. The cursor links, the column sort links, the search box and the filter panel's "Apply" all point `hx-target` at `cr-view-table`, so turning a page, re-sorting, searching or applying a filter replaces the results and nothing else. The visible consequence is the filter panel: it is not re-rendered, so it stays open across an apply with what was typed into it still in it, where it used to close on every reload. They are ordinary links and an ordinary `method="get"` form underneath, and `hx-push-url="true"` puts the request's own URL in the address bar, so the enhanced path ends on the same URL the unenhanced one navigates to — which is what keeps every filtered, sorted and searched state shareable. `hx-swap` is stated in full rather than inherited, because both halves of it would otherwise be wrong: a boosted element defaults to `innerHTML` while the answer carries the region's own root element, and `htmx.config.scrollIntoViewOnBoost` would scroll the results to the top of the viewport — right for a page turn, wrong for an apply, which would push the panel that submitted it off the screen. The sort and cursor links carry ids so that htmx, which restores focus after a swap by looking the previous element's id up, returns the reader to the control they activated instead of to the top of the document; the same swap re-renders that control's label, so a screen reader reads "sort by name descending" out of the element it is still on.

Two facts in the heading depend on the results and live outside the region: the record count and the badge counting applied conditions. The results fragment carries both as `hx-swap-oob` elements, which htmx patches into place by id and then drops from the content it swaps. They are rendered by the same functions the heading calls, with the marker as the only difference, so the patch cannot drift from the page it patches — and the marker is never present in a whole document, where a boosted navigation would apply it and then deliver the page with the element missing. Widening the region to include the heading was the alternative, and it defeats the purpose, because the filter panel is in the heading. Moving a Kanban card is deliberately not one of these controls: the board is the region and a page turn already swaps it, but a move is a `POST` whose successful answer is a redirect, and only the rendered move form could use a second success shape that returned markup — the drag-and-drop equivalent submits with `form.submit()` and is never an htmx request, so the two ways of moving a card would stop behaving alike.

The view index sends its rows before it counts them. Every number on `/` — what each view shows, when the newest of those records changed, the total in the heading — comes from reading every record of every collection, and `/` is the page a server's address opens, so a slow count used to be a slow start. The document now renders the rows with placeholders and marks `cr-view-index` `aria-busy`; the region carries `hx-get="/?summary=inline" hx-trigger="load"`, so once htmx has processed it the region asks for itself and is answered with the same rows counted, the heading's total riding beside it out of band exactly as a view's record count does. The counted region carries no trigger, so it asks once. `/?summary=inline` without an htmx target is the whole document with the numbers in it, which is what the `<noscript>` **Count records** link beside the total offers a browser that will never ask, and what the region is cut from, so the two cannot differ. Unknown query parameters on `/` are ignored rather than refused, unlike every other page, because a stray one on a bookmark must not turn the landing page into an error.

A refused write answers with the form, not with an error page. This is the part of the UI where losing work was easy: a create or update the database refused became the generic error page, and recovering meant navigating back to a `<textarea>` a browser may or may not have kept. The same two envelopes apply. A plain `POST` — which is what a browser with JavaScript off sends — is answered with the whole page and the status of the refusal, the form filled in. An htmx submission names `cr-record-form` in `HX-Target`, because the form points `hx-target` at itself, and is answered with that one element: the identical markup, so the document and the fragment cannot drift, and the breadcrumb, heading and record activity around it are neither re-rendered nor disturbed.

What comes back is the submitted text, escaped, never a reparsed round trip of it: `12500.50` is not answered with `12500.5`, a YAML mapping keeps the order it was typed in, and the Markdown body comes back whole. The version the form was submitted with is preserved too, rather than refreshed to the record's current one, so a save that lost a race cannot be turned into a silent overwrite by pressing the button again. Diagnostics go where the value was typed: `Database::schema_violations` reports each schema failure with the dotted front matter path the schema locates it in — the same derivation `cr check` records in a finding's `field` — and the form puts each one beside that control, marks the control `aria-invalid`, and leaves anything the schema locates in the record as a whole at the top of the form rather than guessing at a field. Errors the server itself raises while reading the form carry the field they are about for the same reason. Nothing about atomicity changes: a refused mutation still writes no record and no audit event, and the status is still the status the JSON API gives the same request — `422`, `412`, `409` or `400` — rather than a flat `422` chosen for the browser's convenience.

Success is two envelopes as well: `303 See Other` for a plain post, and `204 No Content` with `HX-Location` for an htmx one, because an `XMLHttpRequest` follows a redirect invisibly and htmx would swap the destination in while pushing the posted path — `/deals/records`, which answers no `GET` — into the address bar. A refusal says `HX-Push-Url: false` for the same reason.

That contract is what lets the record form be boosted, and `hx-disabled-elt` is part of it: an HTML form post is deliberately outside the `Idempotency-Key` contract the JSON API offers, so the submit button is disabled for the life of the request rather than two clicks becoming two writes. The other elements opt out with `hx-boost="false"`. Links to a JSON representation — `/openapi.json` and the JSON-API buttons — exist to leave the UI, and boosting them would pour a JSON document into the page body as text. Three mutating forms stay native, and two of them now share one reason: the delete form on the confirmation page and the save-as-view form both answer a refusal with a rendered error document, which htmx will not swap into a failed `POST`, so boosting either would turn a lost race or a name already taken into a button that visibly does nothing. The third is the Kanban move form, whose drag-and-drop equivalent builds and submits a form with `form.submit()`, which fires no submit event at all. Two places relax the no-swap rule htmx applies to responses that are not successes, and `cr.js` states both: a boosted `GET` *whose target is `<body>`* that comes back as an HTML error page is swapped in, so a click on a link to a deleted record lands on the same error page a full navigation would have produced — the target is checked as well as the boost because a view's targeted controls are boosted elements that override `hx-target`, and without it a failed re-sort would paste a whole error document inside the table; and any HTML answer carrying `CR-Form-Invalid` is swapped, which is how a refused form reaches the page. That header exists rather than a list of statuses in the browser because the status of a refusal varies while the answer does not, and a listener enumerating statuses would quietly stop covering a route that learned to refuse for a new reason.

Announcements are one region and one sentence. A swap is silent by construction: the rows behind an open filter panel become different rows with no reload, no focus change and nothing said, and for a filter apply focus correctly stays on the button that submitted it, so nothing at all reports the result. The shell therefore renders a single `role="status"` region, visually hidden, empty, and — the load-bearing part — outside `main-content`, `cr-view-table` and `cr-record-form` alike. A live region is announced because an assistive technology is watching *that element*, so a region delivered inside a fragment would be created and filled in one step, which is the case nothing is specified to announce. The results fragment patches it with `hx-swap-oob="innerHTML"` rather than the `"true"` its two heading passengers use, for the same reason: `"true"` replaces the element, and replacing the node being watched with an identical one carrying text is how a region goes quiet. It is empty in every document, because arriving on a page is not a change; the sentence states the range the pager prints, derived from the same `page_range`, so the spoken and the printed cannot drift. The success notice is the one message that arrives *with* a page rather than with a swap, which is exactly the case a live region does not reliably announce, so it is plain markup and `cr.js` copies its text into the region one task later — a mutation of an element the browser has already registered. Two consequences follow from "one region": the notice banner and the impersonation strip gave up their own `role="status"`, the latter because it states a standing fact that `hx-boost` re-created on every navigation. The accessibility tree, not the markup, is what was checked; `.context/phase-5/verification.txt` records the computed role, `live`, `atomic` and text after every swap, and the DOM node identity that proves the element survived it.

Deleting a record is a question the server asks. The record page renders a link; the confirmation page renders the form, the CSRF token and the record's version, so no markup anywhere deletes on a single click. Both are the same path — `GET` asks and `POST` writes, and the `POST` contract is unchanged — and Cancel is a link back to the record, so declining is a navigation rather than a cancelled event. This replaced an inline `onsubmit` calling `window.confirm`, which was worth less than it looked: the handler is JavaScript, so a browser with scripting disabled deleted the record on the first click with nothing asked, and `hx-confirm` would have preserved that hole precisely, being JavaScript too. A page can also say more than a `confirm` string: which record, out of which collection, what survives in the audit log, and a way out that is not a button inside a modal. The version the form carries is read when the question is rendered rather than when the record page was, which narrows — and does not close — the window the `412` exists for. The interaction is the same length it always was: one click to ask, one to confirm.
...
Table pages also derive two columns from the journal: `$created_at` and
`$updated_at`, the first and last events of a record's current lifecycle. A
deletion ends that lifecycle, so an ID created again starts a new `created_at`
rather than inheriting a tombstone's. They are computed by one verified chain
walk per page rather than cached in front matter a direct edit could
contradict, ordered by sequence rather than by formatted instant (two events can
share a second, and RFC 3339 fractional seconds do not sort as text), and
filtered by the same audit-read permission as `cr audit log`, so a timestamp
never reveals history a principal may not read. Records with no history at all —
written directly and not yet saved — carry no derived age and sort last. The
shared record comparator refuses these two fields by name, which keeps the
journal replay in the one layer that already holds the activity map.
...
- Audit replay proves that each event's semantic post-state reproduces its exact record hash, using a versioned exact-Markdown witness where rendering would lose representation details. Every mutation and pending recovery performs that replay before extending the journal. This does not authenticate non-state metadata or an internally consistent rewritten head; an external signed or remote checkpoint remains the boundary for those attacks.
- A read, and a write within `audit.full_walk_after_events` events of the last write that walked from the first event, verifies only what was appended since the walk the last write saved, trusting a sealed segment's file identity and times in place of re-hashing it, and a running server does the same for its own walk. A same-length rewrite that also restores the segment's change time goes unnoticed until the next walk from the first event, and a forged saved walk is believed until then, which for writes is at most that many events; `audit verify`, `check`, and `--verify-audit` still verify the whole chain. See [the verified journal](#the-verified-journal).
- A single-record write verifies the chain once, under the audit lock, and its append checks the journal against that walk as a cached walk is checked rather than walking it again. The lock keeps cooperating writers out of the milliseconds in between; anything else that changes a verified segment sends the append back to the first event, except the same-length rewrite that restores a sealed segment's change time, which the write appends to and the next walk from the first event reports. See [the audit protocol](#audit-protocol).
