...
The HTML layer follows the web platform directly. Native forms submit mutations with `POST`, successful writes use `303 See Other` to return to an idempotent `GET`, and Maud escapes dynamic text while producing server-rendered markup. Enhancement is layered on top of that rather than substituted for it: a vendored copy of htmx boosts navigation so a link swaps the body instead of rebuilding the document, and every route still answers a browser with JavaScript switched off, because htmx intercepts markup that already works. Tailwind's Play CDN supplies the requested no-build styling, with its documented development-only limitation recorded as technical debt.
...
Two kinds of element opt out with `hx-boost="false"`. Links to a JSON representation — `/openapi.json` and the JSON-API buttons — exist to leave the UI, and boosting them would pour a JSON document into the page body as text. Every mutating form stays a native browser submission, because a mutation's current answer is either a `303 See Other` an `XMLHttpRequest` would follow invisibly while htmx pushed the posted path into the address bar, or a rendered error page with a status htmx declines to swap, which would turn a rejected write into a button that visibly does nothing. Redefining that contract — a `422` carrying the re-rendered form, an `HX-Location` on success, `hx-confirm` in place of the delete form's `onsubmit` — is deliberately separate work. One place does relax the rule: a boosted `GET` that comes back as an HTML error page is swapped in anyway, so a click on a link to a record that no longer exists lands on the same error page, at the same URL, that a full navigation would have produced.
