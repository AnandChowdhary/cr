...
The pages' own JavaScript is an embedded asset rather than markup. One file, `src/static/cr.js`, holds the filter builder, the save-as-view layout control, and Kanban drag and drop as separately guarded immediately-invoked functions, is compiled in with `include_str!`, and is linked from `<head>` with `defer` so it still runs against a parsed document. `GET /static/{file}` matches the requested name against the constants the binary was built with and performs no filesystem access, so a traversal has nothing to traverse and the hardened path walk in `src/paths.rs` has no static-file equivalent to maintain. The served name embeds a truncated SHA-256 of the contents, which is what makes `Cache-Control: public, max-age=31536000, immutable` honest: an edit to the script changes its URL, and a version number nobody remembered to raise cannot strand a browser on a stale copy. The route sits outside the authorization layer alongside `/health`, because it carries no database data and because a `<script src>` cannot attach the bearer header `CR_API_TOKEN` requires, so an authenticated asset route would silently disable the enhancements in precisely the deployments that set a token. Keeping the scripts out of the documents is also the prerequisite for a `script-src 'self'` content security policy, which the inline blocks made unreachable.
