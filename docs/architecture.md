...
View filter forms submit repeated `filter_field` and `filter_value` query pairs. The server requires a one-to-one pairing, caps each request at 20 conditions, parses every value with the same typed equality semantics as CLI `list`, and combines them with AND before searching or paginating. Generated controls use schema enum, boolean, number, and string-format metadata; a small progressive script adds/removes rows and swaps the value control when a field changes, but URLs and server behavior do not depend on that script. Pagination preserves all pairs in order.
